...
pub fn _A1m1f(eps: f64, geodesic_order: usize) -> f64 
{
    ...
    let t = polyval(m as isize, &COEFF, eps.sq()) / COEFF[(m + 1) as usize];
    (t + eps) / (1.0 - eps)
}
...
pub fn _C1f(eps: f64, c: &mut [f64], geodesic_order: usize) 
{
    ...
    for l in 1..=geodesic_order 
    {
        ...
        c[l as usize] =
            d * polyval(m as isize, &COEFF[o as usize..], eps2) / COEFF[o + m + 1];
        o += m + 2;
        d *= eps;
    }
}
...
pub fn _A2m1f(eps: f64, geodesic_order: usize) -> f64 
{
    ...
    let m = geodesic_order / 2;
    let t = polyval(m as isize, &COEFF, eps.sq()) / COEFF[m + 1];
    (t - eps) / (1.0 + eps)
}
